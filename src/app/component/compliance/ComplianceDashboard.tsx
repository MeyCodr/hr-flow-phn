...
export default function ComplianceDashboard({
  ...
  return (
    <div className="grid md:grid-cols-3 gap-4">
      ...
      <div className="md:col-span-1 bg-white dark:bg-gray-900 rounded-xl border border-gray-300 dark:border-gray-700 divide-y divide-gray-200 dark:divide-gray-700 max-h-[70vh] overflow-y-auto">
        {reports.length === 0 && (
          <p className="p-4 text-xs text-gray-500 dark:text-gray-400">No reports submitted yet.</p>
        )}
        {reports.map((report) => (
          <button
            key={report.id}
            onClick={() => selectReport(report)}
            className={`w-full text-left p-4 hover:bg-indigo-50 dark:hover:bg-indigo-950/40 transition ${
              selectedId === report.id ? "bg-indigo-100 dark:bg-indigo-900/40" : ""
            }`}
          >
            ...
            <span className="inline-block mt-1 text-xs px-2 py-0.5 rounded bg-gray-200 dark:bg-gray-700 text-gray-700 dark:text-gray-300">
              {report.status}
            </span>
          </button>
        ))}
      ...
      <div className="md:col-span-2 bg-white dark:bg-gray-900 rounded-xl border border-gray-300 dark:border-gray-700 p-6">
        {!selected ? (
          <p className="text-xs text-gray-500 dark:text-gray-400">Select a report to view details.</p>
        ) : (
              ...
              <div>
                ...
                {selected.attachments.map((att) => (
                  <p
                    ...
                    className="cursor-pointer rounded-lg bg-gray-50 dark:bg-gray-800 p-2 border border-gray-300 dark:border-gray-600 hover:bg-indigo-50 dark:hover:bg-indigo-950/40 hover:text-indigo-800 dark:hover:text-indigo-400 transition mt-1"
                  >
                    📎 {att.fileName}
                  </p>
                ))}
            ...
            <div className="border-t border-gray-200 dark:border-gray-700 pt-4">
              ...
              <div className="flex flex-col space-y-2 mb-3">
                ...
                <select
                  ...
                  className="border border-gray-300 dark:border-gray-600 rounded-md px-3 py-2 w-full bg-white dark:bg-gray-800"
                >
